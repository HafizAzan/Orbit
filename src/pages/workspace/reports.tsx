...
function WorkspaceReportsContent() {
  const { data, isLoading, isError } = useWorkspaceReports();
  ...
  if (isError || !data) {
    return (
      <WorkspaceNotFound
        ...
        description="We could not load workspace reports. Please try again shortly."
      />
    );
  }
          ...
          <div className="mt-4 space-y-3">
            {data.tasksByProject.length === 0 ? (
              ...
              data.tasksByProject.map((project) => (
                <div key={project.projectId} className="flex items-center justify-between gap-3 rounded-xl border border-border px-4 py-3">
                  <div>
        ...
        <section className="rounded-2xl border border-border bg-card p-5 shadow-sm">
          ...
          <div className="mt-4 space-y-3">
            {data.tasksByPriority.map((item) => (
              <div key={item.priority} className="flex items-center justify-between gap-3 rounded-xl border border-border px-4 py-3">
