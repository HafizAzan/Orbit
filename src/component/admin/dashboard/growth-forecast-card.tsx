...
function GrowthForecastCard({
  forecast = { target: "—", helperText: "No forecast yet." },
}: {
  forecast?: { target: string; helperText: string };
}) {
  return (
    <article className="rounded-2xl border border-slate-800 bg-slate-900 p-5 text-white shadow-sm">
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <Text as="p" className="mt-2 text-3xl font-bold">{forecast.target}</Text>
          <Paragraph size="sm" className="mt-2 mb-0! text-slate-400">{forecast.helperText}</Paragraph>
        </div>
      </div>
    </article>
  );
