...
function ProjectActivityFeed({ items }: ProjectActivityFeedProps) {
  return (
    <article className="rounded-2xl border border-border bg-card p-5 shadow-sm lg:p-6">
      <Title level={5} color="default">Activity Feed</Title>

      {items.length === 0 ? (
        <Paragraph size="sm" className="mt-5">No task activity yet for this project.</Paragraph>
      ) : (
          ...
          <li key={item.id} className="flex gap-3">
            ...
            <div className="min-w-0 flex-1">
              <Text as="p" size="sm">
                ...
                {item.target ? <Text as="span" weight="semibold"> {item.target}</Text> : null}
              </Text>
              {item.comment ? (
