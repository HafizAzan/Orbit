...
function Title({ level = 1, color = "default", font = "sans", className, children, ...props }: TitleProps) {
  const Tag = titleTags[level];

  return (
    <Tag className={cn(titleStyles[level], colorStyles[color], fontStyles[font], className)} {...props}>
      {children}
    </Tag>
  );
}
...
function Paragraph({ size = "base", color = "muted", font = "sans", className, children, ...props }: ParagraphProps) {
  return (
...
function Text({
  ...
  return (
    <Tag className={cn(textSizes[size], colorStyles[color], fontStyles[font], weightStyles[weight], className)} {...props}>
      {children}
    </Tag>
  );
}
...
function Label({ color = "default", font = "sans", required, className, children, ...props }: LabelProps) {
  return (
    <label className={cn("text-sm leading-6 font-medium", colorStyles[color], fontStyles[font], className)} {...props}>
      {children}
...
export { Title, Paragraph, Text, Label };
