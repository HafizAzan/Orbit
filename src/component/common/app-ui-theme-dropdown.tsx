...
import { APP_UI_THEMES, type AppUiThemeId } from "../../data/app-ui-themes";
  ...
  const content = (
    <div className="w-[min(100vw-2rem,24rem)] p-1">
      ...
      <Paragraph size="sm" color="muted" className="px-2 pb-3">
        Light themes keep the Classic layout — only buttons and accents change. Dark is separate.
      </Paragraph>
        ...
        {APP_UI_THEMES.map((theme) => {
          ...
          return (
            ...
            >
              <div
                className={cn(
                  "mb-2 overflow-hidden rounded-lg border border-border",
              ...
              >
                ...
                <div className="flex gap-1 p-1.5">
                  <div
                    className={cn("h-3 flex-1 rounded", isDark ? "bg-[#151c2f]" : "bg-white")}
                  />
              ...
              {isSelected ? (
                <CheckOutlined className="absolute top-2 right-2 text-primary" />
              ) : null}
            </button>
          );
        })}
